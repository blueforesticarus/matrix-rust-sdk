...
use async_rx::{Vector, VectorSubscriber};
...
impl<P: ProfileProvider> TimelineInner<P> {
    pub(super) fn new(profile_provider: P) -> Self {
        Self { state: Default::default(), profile_provider }
    }

    /// Get a copy of the current items in the list.
    ///
    /// Cheap because `im::Vector` is cheap to clone.
    pub(super) async fn items(&self) -> im::Vector<Arc<TimelineItem>> {
        self.state.lock().await.items.clone()
    }

    pub(super) async fn items_stream(&self) -> VectorSubscriber<Arc<TimelineItem>> {
        trace!("Creating timeline items signal");
        self.state.lock().await.items.subscribe()
    }
